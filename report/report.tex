...
but they are prone to \emph{logical self-contradiction} across related
...
propose a middle ground using \emph{Logic Tensor Networks} (LTNs).  LTNs
are a neuro-symbolic framework that turns first-order logic formulas into
differentiable tensor computations.  They rely on \emph{fuzzy logic}, a
...
\emph{Sentential Decision Diagrams} (SDDs).  An SDD is a compact data
...
SDDs are principled, but they are strictly \emph{propositional}: they
...
\emph{pMeanError} operator, a differentiable generalisation of the
...
provides a stable ``product configuration'': the product t-norm for
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Universal}: a single-antecedent implication (same form as
        legacy rules, but discovered via augmentation).
\end{itemize}
...
Train/validation/test splits are done \emph{by entity} (not randomly by
...
We use the \emph{stable product configuration} from
...
We evaluate along two axes: \emph{factual accuracy} and \emph{logical
consistency}.
...
\emph{Average rule satisfaction} is the mean fuzzy truth value across all
...
semantic loss (\cref{eq:sdd}) works in an \emph{exact probabilistic}
regime: it enumerates all satisfying worlds, so the gradient signal is
...
F1 actually \emph{drops below the untrained base} on prompt~0
...
consistency fall short of SDD-only.  We attribute this to \emph{loss
...
The takeaway is that \emph{exact} probabilistic constraints (SDDs) still
beat \emph{approximate} fuzzy methods (LTNs) when the rules fit in
